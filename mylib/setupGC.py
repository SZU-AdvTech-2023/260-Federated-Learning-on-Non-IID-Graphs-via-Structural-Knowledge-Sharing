...
# 用于准备多个图数据集的训练、验证和测试数据集，并返回有关这些数据集的一些统计信息，
# 函数返回一个包含有关不同数据集的统计信息的 DataFrame，该 DataFrame 可以帮助你了解数据集的规模、图的平均节点数和边数等信息
...
# 创建了客户端和服务器对象，其中客户端包含了图神经网络模型 (Client_GC 类)，而服务器包含了对应的聚合模型 (Server 类)，对于每个数据集，都创建了一个客户端
